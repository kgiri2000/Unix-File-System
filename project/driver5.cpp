...
int main()
{
  ...
  r = c1->myFS->createDirectory(const_cast<char *>("/e"), 2);
  cout << "rv from createDirectory /e is " << r <<(r==-1 ? " correct /e dir exists": " fail") <<endl;
  r = c1->myFS->createDirectory(const_cast<char *>("/a"), 2);
  cout << "rv from createDirectory /a is " << r <<(r==-1 ? " correct /a file exists": " fail") <<endl;
  r = c1->myFS->createFile(const_cast<char *>("/e"), 2);
  cout << "rv from createFile /e is " << r <<(r==-1 ? " correct": " fail") <<endl;
  r = c1->myFS->createFile(const_cast<char *>("/e/a"), 4);
  cout << "rv from createFile /e/a is " << r <<(r==0 ? " correct": " fail") <<endl;
  r = c1->myFS->createDirectory(const_cast<char *>("/e/b"), 4);
  cout << "rv from createDirectory /e/b is " << r <<(r==0 ? " correct": " fail") <<endl;
  r = c1->myFS->createFile(const_cast<char *>("/e/b/a"), 6);
  cout << "rv from createFile /e/b/a is " << r <<(r==0 ? " correct": " fail") <<endl;

  r = c1->myFS->createFile(const_cast<char *>("/e/g/a"), 6);
  cout << "rv from createFile /e/g/a is " << r <<(r==-4 ? " correct no /e/g/ dir": " fail") <<endl;
  r = c1->myFS->createDirectory(const_cast<char *>("/e/g/a"), 6);
  cout << "rv from createDirectory /e/g/a is " << r <<(r==-4 ? " correct no /e/g/ dir": " fail") <<endl;
  r = c1->myFS->createDirectory(const_cast<char *>("/e/g/b"), 6);
  cout << "rv from createDirectory /e/g/b is " << r <<(r==-4 ? " correct no /e/g/ dir": " fail") <<endl;
  ...
  cout << "rv from createFile /e/b is " << r <<(r==-1 ? " correct /e/b is a dir": " fail") <<endl;
  r = c1->myFS->createFile(const_cast<char *>("/e/B"), 4);
  cout << "rv from createFile /e/B is " << r <<(r==0 ? " correct": " fail") <<endl;
  r = c1->myFS->createFile(const_cast<char *>("/e/a"), 4);
  cout << "rv from createFile /e/a is " << r <<(r==-1 ? " correct, already exists": " fail") <<endl;
  r = c1->myFS->createDirectory(const_cast<char *>("/e/a"), 4);
  cout << "rv from createDirectory /e/a is " << r <<(r==-1 ? " correct file /e/a exists": " fail") <<endl;
  r = c1->myFS->createDirectory(const_cast<char *>("/e/A"), 4);
  ...
  cout << "rv from createFile /e/A/a is " << r <<(r==-1 ? " correct dir /e/A/a exists": " fail") <<endl;
  ...
  cout << "rv from openFile is /e/a " << f2 <<(f2>0 ? " correct, open file /e/a": " fail") <<endl;
  r = c1->myFS->closeFile(f2);
  cout << "rv from closeFile /e/a is " << r <<(r==0 ? " correct": " fail") <<endl;
  ...
  cout << "rv from openFile is /e/$/a/b " << f3 <<(f3==-4 ? " correct": " fail") <<endl;
  ...
  cout << "rv from deleteDirectory /g/a/g/x is " << r <<(r==-1 ? " correct x is file": " fail") <<endl;
  ...
  r = c1->myFS->createDirectory(const_cast<char *>("/i"), 2);
  cout << "rv from createDirectory /i is " << r <<(r==0 ? " correct": " fail") <<endl;
  l1 = c1->myFS->lockFile(const_cast<char *>("/h"), 2);
  cout << "rv from lockFile /h is " << l1 <<(l1>=0 ? " correct /h is locked": " fail") <<endl;
  r = c1->myFS->deleteFile(const_cast<char *>("/h"), 2);
  cout << "rv from deleteFile /h is " << r  <<(r==-2 ? " correct /h file is locked": " fail") <<endl;
  r = c1->myFS->deleteFile(const_cast<char *>("/i"), 2);
  cout << "rv from deleteFile /i is " << r  <<(r==-1 ? " correct /i is a dir": " fail") <<endl;

  cout <<"\n rename tests on directories fs1\n";
  r = c1->myFS->renameFile(const_cast<char *>("/h"), 2, const_cast<char *>("/x"), 2);
  cout << "rv from renameFile /h /x is " << r <<(r==-3 ? " correct /x exists (also /h is locked)": " fail") <<endl;
    r = c1->myFS->renameFile(const_cast<char *>("/h"), 2, const_cast<char *>("/y"), 2);
  cout << "rv from renameFile /h /x is " << r <<(r==-4 ? " correct /h is locked": " fail") <<endl;
  r = c1->myFS->renameDirectory(const_cast<char *>("/H"), 2, const_cast<char *>("/i"), 2);
  cout << "rv from renameFile /H /i is " << r <<(r==-3 ? " correct / dir exists": " fail") <<endl;
  r = c1->myFS->renameDirectory(const_cast<char *>("/$"), 2, const_cast<char *>("/z"), 2);
  cout << "rv from renameFile /$ /x is " << r <<(r==-1 ? " correct /$  invalid": " fail") <<endl;
  r = c1->myFS->renameDirectory(const_cast<char *>("/H"), 2, const_cast<char *>("/o"), 2);
  cout << "rv from renameFile /H /o is " << r <<(r==0 ? " correct": " fail") <<endl;
  r = c1->myFS->renameDirectory(const_cast<char *>("/i"), 2, const_cast<char *>("/o"), 2);
  cout << "rv from renameFile /i /o is " << r <<(r==-3 ? " correct /o exists": " fail") <<endl;
  
  r = c1->myFS->unlockFile(const_cast<char *>("/h"),2,l1);
  cout << "rv from unlockFile /h is " << r <<(r==0 ? " correct": " fail") <<endl;
  r = c1->myFS->renameFile(const_cast<char *>("/h"), 2, const_cast<char *>("/o"), 2);
  cout << "rv from renameFile /h /o is " << r <<(r==-3 ? " correct /o dir exists": " fail") <<endl;
  r = c1->myFS->renameFile(const_cast<char *>("/h"), 2, const_cast<char *>("/O"), 2);
  cout << "rv from renameFile /h /O is " << r <<(r==0 ? " correct": " fail") <<endl;
  ...
  r = c1->myFS->renameDirectory(const_cast<char *>("/e/A/a"), 6, const_cast<char *>("/e/A/z"), 6);
  cout << "rv from renameDirectory /e/a/a /e/a/z is " << r <<(r==0 ? " correct": " fail") <<endl;
  r = c1->myFS->renameDirectory(const_cast<char *>("/g/A/g"), 6, const_cast<char *>("/g/A/x"), 6);
  cout << "rv from renameDirectory /g/a/g /g/a/x is " << r <<(r==-2 ? " correct doesn't exist.": " fail") <<endl;

  cout <<"end of driver 5\n";
