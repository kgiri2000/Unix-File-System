...
int main()
{
  ...
  cout <<"rename test on all fileystems\n";
  ...
  cout << "rv from renameFile /B to /b fs3 is " << r <<(r==0 ? " correct ": " fail") <<endl;
  r = c4->myFS->renameFile(const_cast<char *>("/B"), 2, const_cast<char *>("/C"), 2);
  cout << "rv from renameFile /B to /C fs4 is " << r <<(r==0 ? " correct ": " fail") <<endl;
  ...
  cout <<"\ndelete a file on fs4\n";  
  r = c4->myFS->deleteFile(const_cast<char *>("/C"), 2);
  cout << "rv from deleteFile /C is fs4" << r <<(r==0 ? " correct":" fail")<<endl;
