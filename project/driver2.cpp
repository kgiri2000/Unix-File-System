...
int main()
{
  ...
  Client *c4 = new Client(fs4);

  int i, r, l1, l2, l3, f1, f2, f3, f4, f5;
  ...
  cout << "rv from createFile /A fs4 is " << r <<(r==0 ? " correct ": " fail") <<endl;
  r = c4->myFS->createFile(const_cast<char *>("/B"), 2);
  cout << "rv from createFile /B fs4 is " << r <<(r==0 ? " correct ": " fail") <<endl;

  cout <<"\nread/write and files on all partitions\n";
  ...
  f5 = c4->myFS->openFile(const_cast<char *>("/A"), 2, 'm', -1);
  cout << "rv from openFile /A m fs4 is " << f5 << (f5>0 ? " correct ": " fail") <<endl;
  ...
  r = c4->myFS->writeFile(f5, buf1, 5);
  cout << "rv from writeFile /A fs4 is " << r <<(r==5 ? " correct ": " fail") <<endl;
  ...
  r = c4->myFS->closeFile(f5);
  cout << "rv from closeFile /A fs4 is " << r << (r==0 ? " correct":" fail")<<endl;
  ...
  r = c4->myFS->readFile(f5, rbuf1, 10);
  cout << "rv from readFile /A fs4 (f5) is " << r << (r==5 ? " correct ": " fail") <<endl;
  cout << "Data read is " << endl;
  for (i = 0; i < r; i++) cout << rbuf1[i];
  cout << endl;
  r = c4->myFS->closeFile(f5);
  cout << "rv from closeFile /A fs4 (f5) is " << r << (r==0 ? " correct":" fail")<<endl;
  ...
  l3 = c4->myFS->lockFile(const_cast<char *>("/A"), 2);
  cout << "rv from lockFile /A fs4 is " << l3 << (l3>0 ? " correct": " failure") <<endl;
  r = c4->myFS->unlockFile( const_cast<char *>("/A"),2,l3);
  cout << "rv from unlockFile /A fs4 is " << r << (r==0 ? " correct ": " failure") <<endl;
